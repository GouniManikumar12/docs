...
- **Tail-based display system** for recommendations
...
<AccordionGroup>
  <Accordion title="Backend (Python + FastAPI)">
    ...
    try:
        ...
        return [
            RecommendationResponse(
                title=rec['title'],
                reason=rec.get('meta', {}).get('reason', rec.get('reason', '')),
                intent_match_score=rec.get('meta', {}).get('intent_match_score', 0.0),
                admesh_link=rec['admesh_link'],
                ad_id=rec.get('meta', {}).get('ad_id', rec.get('ad_id', ''))
            )
