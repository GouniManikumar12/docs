...
**Formats supported:**
- `format="tail"` - Inline tail with product links
- `format="product"` - Product cards with details
- `format="bridge"` - Followup sponsored recommendations for Vibe Coding Platforms, AI IDEs, and AI search (setup prompts and documentation URLs)
...
<WeaveAdFormatContainer
  messageId={msg.id}
  fallbackUI={<AdMeshRecommendations messages={[msg]} format="tail" />}
>
  {msg.content}
</WeaveAdFormatContainer>
```
...
- Multiple format options (citation, product cards, bridge format for followup sponsored recommendations)
...
| **Formats** | Citation, Product, Bridge (followup sponsored recommendations) | Weave Ad Format |
