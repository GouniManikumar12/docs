...
### AI Platform Runtime Integration

If your product is an AI assistant, AI search product, or chat platform, there is one extra layer beyond rendering recommendations:

- use `AdMeshRecommendations` to show sponsored recommendation units
- use the SDK delegation callbacks to let users start in-session brand conversations
- route delegated turns through your own host runtime after activation

See [Bridge Format](/platforms/bridge-format) for the full host/runtime model, consent handling, and delegated session lifecycle.
...
3. **For AI platform runtime integration:** Go to [Bridge Format](/platforms/bridge-format)
4. **For Flutter frontend setup:** Go to [Flutter Frontend SDK](/ui-sdk/flutter)
5. **Have Questions?** Check our [FAQ](/platforms/faq) or contact [support@useadmesh.com](mailto:support@useadmesh.com)
