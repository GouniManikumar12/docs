...
When a recommendation includes `followup_suggestion`, the SDK will automatically inject the sponsored follow-up into your container using React portals. It will appear alongside your existing suggestions, seamlessly integrated into your UI.
...
1. **Detection**: When a recommendation includes `followup_suggestion.query`, the SDK detects it automatically.
...
- Follow-ups are only displayed if the recommendation includes `followup_suggestion` from the backend.
...
- Modify or remove transparency labels ("Sponsored" labels are required)
...
<AccordionGroup>
  ...
  <Accordion title="Follow-Ups Not Showing">
    ...
    **Check:**
    ...
    - Recommendation from backend includes `followup_suggestion.query`, `followup_suggestion.engagement_url`, and `followup_suggestion.exposure_url`
