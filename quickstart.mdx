...
func getRecommendations(query, agentID string) ([]interface{}, error) {
    ...
    req, _ := http.NewRequest("POST", "https://api.useadmesh.com/agent/recommendations", bytes.NewBuffer(jsonData))
...
def get_recommendations(query, agent_id)
  uri = URI('https://api.useadmesh.com/agent/recommendations')
