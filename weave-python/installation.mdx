...
if result["found"]:
    weave_prompt = build_weave_prompt(result["recommendations"][0])
```
...
<admesh_recommendation_data>{recommendation_data}</admesh_recommendation_data>
...
if recommendation and recommendation["click_url"] not in response:
    response = render_configured_fallback(recommendation)
```
...
<Note>
In the canonical AIP wire contract, `message_id` is the per-message identifier used for auction identity and traceability.
</Note>
